...
public class SplineProfileHigh {
	...
	private double width = RobotMap.robotWidth;
	private double rightOutput = 0, leftOutput = 0;
	private boolean angleInverted = false, splineBackwards = false, rightOuter;
	
	public SplineProfileHigh(double Kp, double Ki, double Kd, double Ka,
			double Kv){
		this.Kp = Kp;
		this.Ki = Ki;
		this.Kd = Kd;
	...
	public void configureSplineProfile(double radius, double angle, boolean direction) {
		this.maxAcc = RobotMap.maxHighAcceleration;
	...
	public void calculate(double leftDistance, double rightDistance){
		...
		double currentAngle = Math.abs(CommandBase.chassis.getSpartanGyro() - this.startGyroAngle);
		if(this.splineBackwards){
			currentAngle *= -1;
		}
